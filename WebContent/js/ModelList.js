...
function showModelFiles(source)
{
	...
	if(modelfiles.style.display=="none"){//如果模型文件部分为隐藏状态
		...
		$.ajax({
		     type: "POST",   
		     url: "/NodesoftManage/GetModellist?objId="+TargetObjectID,   
...
function jianmo(){
	...
	if(ModelfileID){
		$.ajax({
			type: "post",
			url:'/NodesoftManage/GetAvailableSoft?fileID='+ModelfileID,//模型文件ID（文件名fileinfo_ID）
			success:function(data){
...
function insertNewUserRecord()
{
	...
	if(checkInput()){
		...
		$.ajax({   
		     type: "POST",   
		     url: "/NodesoftManage/CreateNewRecord?type=object",   
		     data:jsonObject,
		     //traditional: true,
