...
if(session.getAttribute("username")==null) {
    response.sendRedirect("/NodesoftManage/login.jsp?frompage=ModelList.jsp");
 }
else if (request.getAttribute("Modellist") == null) {
%>
<jsp:forward page="/GetModellist">
...
<title>目标特性建模软件及几何模型管理平台——目标信息</title>
			...
			<table id="myTable" border="0">
				...
				<tbody id="filename">
					...
					<tr onclick="showModelFiles(this)">
						<td><strong>NO.</strong>${item[0]}<input type="hidden" value="${item[0]}"></td>
						<td><a href="#" onclick="ModifyObjectinfo(${item[0]})">${item[1]}</a></td>
						<td>${item[2]}.${item[3]}</td>
						<td>${item[4]}</td>
						<!-- <td><span onclick="viewmodel(${status.index})"><a href="#" >查看模型</a></span></td> -->
						<td><span onclick="addModelFile(${item[0]},'${item[1]}')"><a href="#" >添加模型</a></span></td>
					</tr>
